...
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 1, "pblk_gc_rq *", "gc_rq", gc_rq);
	if (gc_rq->data)
		vfree(gc_rq->data);
	kfree(gc_rq);
}
...
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 1, "pblk_gc *", "gc", gc);
...
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 2, "pblk *", "pblk", pblk, "list_head *", "group_list", group_list);
...
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 2, "pblk_gc *", "gc", gc, "pblk_rl *", "rl", rl);
...
void pblk_gc_sysfs_state_show(struct pblk *pblk, int *gc_enabled,
			      int *gc_active)
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 3, "pblk *", "pblk", pblk, "int *", "gc_enabled", gc_enabled, "int *", "gc_active", gc_active);
	struct pblk_gc *gc = &pblk->gc;
...
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 2, "pblk *", "pblk", pblk, "int", "force", force);
	struct pblk_gc *gc = &pblk->gc;

...
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 2, "pblk *", "pblk", pblk, "char *", "page", page);
...
static ssize_t pblk_get_write_amp(u64 user, u64 gc, u64 pad,
				  char *page)
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 4, "u64", "user", user, "u64", "gc", gc, "u64", "pad", pad, "char *", "page", page);
	int sz;
...
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 2, "unsigned long long", "bucket", bucket, "unsigned long long", "total", total);
	int p = bucket * 100;
...
static ssize_t pblk_sysfs_gc_force(struct pblk *pblk, const char *page,
				   size_t len)
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 3, "pblk *", "pblk", pblk, "const char *", "page", page, "size_t", "len", len);
	size_t c_len;
	int force;
...
static ssize_t pblk_sysfs_show(struct kobject *kobj, struct attribute *attr,
			       char *buf)
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 3, "kobject *", "kobj", kobj, "attribute *", "attr", attr, "char *", "buf", buf);
...
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 4, "kobject *", "kobj", kobj, "attribute *", "attr", attr, "const char *", "buf", buf, "size_t", "len", len);
	struct pblk *pblk = container_of(kobj, struct pblk, kobj);
...
{
	CPS_FUNCTION(CPS_CALLER, CPS_CALLEE, __FILE__, __func__, 1, "gendisk *", "tdisk", tdisk);
